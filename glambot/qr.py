...
def make_wifi_qr_data_uri(payload: str) -> str:
    """QR encoding a `WIFI:` join-network string (see glambot/lan.py)."""
    return make_qr_data_uri(payload)


def make_delivery_photo(thumbnail_path: Path, url: str, *, url2: str | None = None,
                         label: str | None = None, label2: str | None = None,
                         wifi_payload: str | None = None) -> bytes:
    ...
    with its own optional caption (e.g. the aspect ratio). When `wifi_payload`
    is given (offline LAN delivery), a "Join Wi-Fi" QR is shown to the left of
    the download QR instead."""
    ...
    if wifi_payload:
        label, label2 = "1. Join Wi-Fi", "2. Scan to download"
        label_h = 30
        qr_size = int(target_w * 0.38)
        gap = margin
        qr1, qr2 = _qr(wifi_payload, qr_size), _qr(url, qr_size)
        row_w = qr_size * 2 + gap
